  ...
  Widget build(BuildContext context) {
    return DraggableScrollableSheet(
      ...
      builder: (context, scrollController) {
        final ratingList = data?.reviews?.where((r) => r.rating != null).toList() ?? [];
        final reviewFilter = data?.reviews?.where((r) => r.rating != null && r.message != null && r.message != "").toList() ?? [];

        final totalRating = ratingList.fold(0.0, (sum, r) => sum + r.rating!);
        final averageRating = ratingList.isNotEmpty ? totalRating / ratingList.length : 0.0;
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                Expanded(
                                  flex: 1,
                                  child: Text(
                                    data?.stock == 0 ? "" : 'Stock : ${data?.stock}',
                                    style: TextStyle(
                                      color: data?.stock == 0 ? AppColors.redColor : AppColors.blackColor,
                            ...
                            data!.reviews!.isEmpty ? Container() : Padding(
                              padding: const EdgeInsets.symmetric(vertical: 10),
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.start,
                                crossAxisAlignment: CrossAxisAlignment.center,
                                  ...
                                  RichText(
                                    text: TextSpan(
                                      children: [
                                        ...
                                        TextSpan(
                                          text: '(${ratingList.length} rating • ${reviewFilter.length} ulasan)',
                                          style: const TextStyle(
                                            color: AppColors.blackColor,
                                            fontSize: fontSizeDefault,
                                            fontWeight: FontWeight.bold,
                                    ...
                                    children: [
                                      ...
                                      Padding(
                                        ...
                                        child: RatingBar.readOnly(
                                          ...
                                          initialRating: riview?.rating?.toDouble() ?? 0.0,
                                          maxRating: 5,
                                        ),
                                      ),
                                      ...
                                      riview?.images?.isEmpty ?? false ? const SizedBox.shrink() : const SizedBox(
                                        height: 10,
                                      ),
                                      riview?.images?.isEmpty ?? false ? const SizedBox.shrink() : SingleChildScrollView(
                                        scrollDirection: Axis.horizontal,
                                        child: Row(
                                          children: List.generate(riview?.images?.length ?? 0,
                                              (index) {
                                            return Container(
                                              width: 100,
                                              height: 100,
