    ...
    return BlocBuilder<OrderCubit, OrderState>(
      builder: (context, state) {
        return state.loading ? 
          const CustomLoadingPage() : state.order.isEmpty ? 
                        ...
                        children: [
                          Column(
                            ...
                            children: [
                              ...
                              Text(DateHelper.parseDate(e.createdAt ?? ""))
                            ],
                          ),
                          Container(
                            padding: const EdgeInsets.all(5),
                            decoration: BoxDecoration(
                              color: e.status == "CANCEL_SYSTEM" ? AppColors.secondaryColor : AppColors.blueColor,
                              borderRadius: BorderRadius.all(Radius.circular(5)),
                              border: Border.fromBorderSide(BorderSide(color: AppColors.whiteColor, width: 1, strokeAlign: 1))
                            ),
                            child: Text(
                              e.status == "ON_PROCESS" ? "Diproses" :
                              e.status == "DELIVERY" ? "Dikirim" :
                              e.status == "DELIVERED" ? "Tiba di tujuan" :
                              e.status == "FINISHED" ? "Selesai" : 
                              e.status == "DELIVERED" ? "Tiba di tujuan" : 
                              e.status == "CANCEL" ? "Dibatalkan" : 
                              e.status == "CANCEL_SYSTEM" ? "Dibatalkan Dari Sistem" : 
                              "Menunggu Konfirmasi",
