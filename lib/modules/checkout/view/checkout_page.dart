  ...
  Widget build(BuildContext context) {
    return BlocProvider.value(
        value: getIt<CheckoutCubit>()
          ..init(context: context, from: from, qty: qty, productId: productId),
        child: CheckoutView(
          from: from,
        ));
  }
            ...
            child: CustomScrollView(
              slivers: [
                ...
                state.loading
                    ...
                    : (state.checkout.isEmpty &&
                            state.checkoutNow?.data == null)
                        ? const SliverFillRemaining(
                            child:
                                Center(child: EmptyPage(msg: "Checkout Empty")))
                        : SliverPadding(
                                              ...
                                              child: RichText(
                                                text: TextSpan(
                                                    ...
                                                    children: [
                                                      const TextSpan(
                                                          text:
                                                              'Anda belum menambahkan alamat, silahkan '),
                                                      TextSpan(
                                                          text:
                                                              "tambah alamat baru",
                                                          style: const TextStyle(
                                                ...
                                                Expanded(
                                                  child: Text(
                                                    state.loadingChannel
                                                        ? "Loading..."
                                                        : state.channel != null
                                                            ? "${state.channel?.name == "Saldo" ? "PGB  Wallet" : state.channel?.name} ${state.channel?.paymentType == "APP" ? '( ${Price.currency(state.channel?.user?.balance?.toDouble() ?? 0)} )' : ""}"
                                      ...
                                      Padding(
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            ...
                                            const SizedBox(height: 4),
                                            Text(
                                              Price.currency(state.totalPrice),
                                              style: const TextStyle(
                                                fontSize: fontSizeDefault,
                                                fontWeight: FontWeight.bold,
                                                color: AppColors.blackColor,
