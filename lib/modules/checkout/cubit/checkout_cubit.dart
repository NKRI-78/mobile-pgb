...
import 'package:mobile_pgb/modules/checkout/widget/cost_shipping.dart';
import 'package:mobile_pgb/modules/checkout/widget/select_payment_channel.dart';
...
class CheckoutCubit extends Cubit<CheckoutState> {
  ...
  Future<void> init({String from = "", String? qty, String? productId, required BuildContext context}) async {
      emit(const CheckoutState());
      getShippingMain();
      if (from == "NOW") {
        getCheckoutNow(from: from, qty: qty, productId: productId, context: context);
      } else {
        fetchCheckout(from: from, qty: qty, productId: productId, context: context);
      }
      emit(state.copyWith(
          from: from,
          qty: qty,
  ...
  void calculateTotal() {
    ...
    double totalPriceProduct = state.checkout.fold(0.0, (sum, checkout) {
        return sum! + ((checkout.carts?.fold(0.0, (sum, cart) {
          return sum + (cart.price ?? 0);
        }) ?? 0) * (checkout.carts?.fold(0.0, (sum, cart) {
          return (cart.quantity ?? 0);
        }) ?? 0));
      }) ?? 0;
    double totalCost = state.shippings?.values.fold(0.0, (sum, item) => sum! + (int.parse(item["cost"]).toDouble())) ?? 0.0;
  ...
  Future<void> fetchCheckout(
      {String from = "", String? qty, String? productId, required BuildContext context}) async {
    try {
      emit(state.copyWith(loading: true));
      var cart = await repo.getCheckout(from: from, qty: qty, productId: productId);

      double totalPriceProduct = cart.fold(0.0, (sum, checkout) {
        return sum + (checkout.carts?.fold(0.0, (cartSum, cart) {
          return cartSum! + ((cart.price ?? 0.0) * (cart.quantity ?? 0));
        }) ?? 0.0);
      });
      double totalCost = state.shippings?.values.fold(
        0.0,
        (sum, item) => sum! + (double.tryParse(item["cost"].toString()) ?? 0.0),
      ) ?? 0.0;
  ...
  Future<void> getCheckoutNow(
    ...
    try {
      ...
      double admin = state.channel?.fee?.toDouble() ?? 0.0;
      double totalPrice = checkoutNow.totalPrice?.toDouble() ?? 0.0 + totalCost + admin;
  ...
  Future<void> getCostItemV2(
    ...
    try {
      ...
      if(cost.isNotEmpty && context.mounted){
        ...
        if(context.mounted){
          ShowSnackbar.snackbar(context, "Anda belum menambahkan alamat pengiriman", isSuccess: false);
        }
      }
